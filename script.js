...
/**
 * =================================================================
 * 2. TRENDY ELITE COLLECTION (UPDATED WITH VERIFIED IMAGES)
 * =================================================================
 */
...
async function init() {
    ...
    try {
        ...
        const filteredProducts = data.products.filter(p => p.category !== "beauty");
    ...
    document.getElementById('close-cart').onclick = window.closeCart;
    document.getElementById('cart-overlay').onclick = window.closeCart;
}
...
function renderCategoryBar() {
    ...
    bar.innerHTML = CATS.map(c => `
        <div class="category-item ${state.selectedCategory === c ? 'active' : ''}" onclick="filterCat('${c}')">
            <div class="category-icon-box glass-card">${getIcon(c)}</div>
            <span class="text-[11px] font-black text-slate-400 uppercase tracking-widest">${c}</span>
        </div>
    `).join('');
}
...
function renderGrid() {
    ...
    grid.innerHTML = filtered.map(p => `
        <div class="glass-card rounded-[40px] p-6 flex flex-col animate-fade">
            <div class="relative h-64 mb-6 rounded-[28px] overflow-hidden bg-slate-100">
                ...
                <div class="absolute top-4 left-4 glass px-3 py-1 rounded-full text-[10px] font-black uppercase text-violet-700 border-violet-100">${p.brand || 'Suite'}</div>
            </div>
...
/**
 * =================================================================
 * 5. GLOBAL ACTIONS (Exposed for HTML)
 * =================================================================
 */
...
window.openCart = function() {
    ...
    // Ensure we start at the bag view whenever it opens
...
// Updated showStep to toggle visibility inside the sidebar exclusively
...
window.qty = (id, d) => {
    const productId = Number(id);
    const i = cart.find(x => Number(x.id) === productId);
    if (i) {
        i.qty += d;
...
// Transition from Address to Payment within the sidebar
window.saveAddressAndGoToPayment = function() {
    ...
    if (!name || !line || !city || !pin) return alert("Please fill all delivery details");
    
    // Update Address Preview text for the payment step
    const previewEl = document.getElementById('address-preview');
    if (previewEl) {
        previewEl.innerHTML = `${name}<br>${line}, ${city} - ${pin}`;
    }
    
    window.showStep('side-step-payment');
    updateCartUI(); // Updates the total in the payment view
};

// Payment selection with ₹100 COD Fee logic and Visual Highlight
window.selectPayment = function(method) {
    state.paymentMethod = method;
    
    // UI Feedback for selection
    document.querySelectorAll('.pay-opt').forEach(el => {
        el.classList.remove('border-violet-500', 'bg-violet-50/30', 'ring-2', 'ring-violet-200');
    });
    
    // Highlight the clicked element
    if (event && event.currentTarget) {
        event.currentTarget.classList.add('border-violet-500', 'bg-violet-50/30', 'ring-2', 'ring-violet-200');
    }

    // Calculate total including 18% GST and potential COD fee
    const subtotal = cart.reduce((a, b) => a + (b.price * b.qty * INR_CONV), 0);
    let total = subtotal * 1.18;


    const finalDisp = document.getElementById('final-amount-display');
    if (finalDisp) finalDisp.innerText = `₹${Math.round(total).toLocaleString()}`;
};

window.finalPurchase = function() {
    if (!state.paymentMethod) return alert("Please select a payment method");
    window.showStep('side-step-success');
    
    // Clear cart data
    cart = [];
    state.paymentMethod = null; // Reset selection
    updateCartUI();
};
...
function updateCartUI() {
    ...
    // TAX LOGIC: Now working correctly
    const subtotal = cart.reduce((a, b) => a + (b.price * b.qty * INR_CONV), 0);
...
function showStep(stepId) {
    ...
    if (container) {
        ...
        // Ensure it uses the existing popup style
        setTimeout(() => {
            container.classList.remove('modal-inactive');
...
// Global scope window functions for HTML onclicks
window.selectPayment = function(method) {
    state.paymentMethod = method;
    document.querySelectorAll('.pay-opt').forEach(el => el.classList.remove('border-violet-500', 'bg-violet-50/30'));
    event.currentTarget.classList.add('border-violet-500', 'bg-violet-50/30');
    
    document.getElementById('emi-calc-box').classList.toggle('hidden', method !== 'emi');
    if(method === 'emi') updateEMIDetails();
};
...
function updateEMIDetails() {
    ...
    document.getElementById('emi-down').innerText = `₹${Math.round(down).toLocaleString()}`;
    document.getElementById('emi-month').innerText = `₹${Math.round(monthly).toLocaleString()} × 9 months`;
}
